...
- `knowledge_search` mode guidance is a behavior contract: `hybrid` default, `fast` exact symbols/files/errors/config, `semantic` conceptual wording, `adaptive` surrounding context for implementation, `deep` high-stakes or ambiguous verification. Empty/weak results should trigger one mode retry before declaring no answer.
