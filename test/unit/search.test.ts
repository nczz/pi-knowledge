...
describe("search pipeline", () => {
	...
	beforeEach(() => {
		...
		const texts = ["SQLite FTS5 full-text search", "Vector embeddings semantic", "認證流程 OAuth token", "React useState state"];
		insertChunks(db, kb.id, texts.map((t) => ({
			content_hash: contentHash(t), content: t, content_tokenized: preTokenizeForFTS(t),
			file_path: "t.md", file_type: "markdown", start_line: 1, end_line: 1, metadata_json: "{}",
		})));
		chunkIds = getChunkIdsByKB(db, kb.id);
	});

	afterEach(() => { db.close(); rmSync(TEST_DIR, { recursive: true, force: true }); });
	...
	describe("Vector search", () => {
		it("top-K sorted", () => {
			...
			const vecs = [new Float32Array([0.9, 0.1, 0, 0]), new Float32Array([0, 1, 0, 0]), new Float32Array([0.5, 0.5, 0, 0]), new Float32Array([0.8, 0.2, 0, 0])];
	...
	describe("RRF", () => {
		it("merges with overlap boosted", () => {
			const l1 = [{ chunkId: "a", score: 1 }, { chunkId: "b", score: 0.5 }];
			const l2 = [{ chunkId: "b", score: 1 }, { chunkId: "c", score: 0.5 }];
